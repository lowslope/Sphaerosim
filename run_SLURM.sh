...
#export TEST_FLAGS_COMPILER="-no-vec -no-simd"
...
if [[ "$RunType" = "likwid" ]]; then
    ...
    export OMP_PLACES=cores
    export OMP_PROC_BIND=close
elif [[ "$RunType" = "VTune mc" ]]; then
    CmakeFlags="-DDebug=1 -DCMAKE_BUILD_TYPE=release"
    RunPrefix="/rwthfs/rz/SW/intel/vtune/XE2019-u02/vtune_amplifier_2019.2.0.584348/bin64/amplxe-cl -collect memory-consumption -data-limit=1024 -app-working-dir $(pwd) -result-dir /work/xu077052/Projekte/sphaerosim/VTune/Sphaerosim/${SLURM_JOB_ID}mc/ --"
    export OMP_PLACES=cores
    export OMP_PROC_BIND=close
